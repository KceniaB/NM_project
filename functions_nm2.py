...
def extract_data_info(df, index): 
    """ 
    extracts in string format the mouse name, date of the session, nph file number, bnc file number and regions
    """
    mouse = df.Mouse.values[index] #"ZFM-06948" 
    date = df['date'].dt.strftime('%Y-%m-%d').values[index] #"2024-03-22"
    nphfile_number = str(df.nph_file.values[index]) #"0"
    bncfile_number = str(df.nph_bnc.values[index]) #"0"
    region = "Region"+str(df.region.values[index])+"G"
    region2 = "Region"+str(df.region2.values[index])+"G" 
