...
ZFM-03059	2021-08-23	0fa78e73-a223-4e55-a524-a839262308e6
    -> many different files 

ZFM-03059	2021-08-24	3bf6c7e2-3eb1-43be-b409-4fed80e46dde
    -> signal jumps, so I divided it into 3 parts: 
        plt.plot(df_nph.raw_calcium[0:30365])
        plt.plot(df_nph.raw_calcium[30450:43000]) 
        plt.plot(df_nph.raw_calcium[46550:54100])
ZFM-03447	2021-11-04	a08a36d0-ca61-4e8a-91d4-16547cc2f49e
    nph_path = '/mnt/h0/kb/data/external_drive/HCW_S2_23082021/DA/HCW_S03_04Nov2021/PhotometryData_D1_HCW_S03_04Nov2021.csv'
    bpod_path = '/mnt/h0/kb/data/external_drive/HCW_S2_23082021/DA/HCW_S03_04Nov2021/DI1_D1_HCW_S03_04Nov2021.csv'
    regions = 'Region1G'
...
#D6
HCW='HCW1'
eid = '53ffd1ce-4028-4c83-a4be-d71da024d4bf' 
nph_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D4_D5_D6_HCW1/PhotometryData_D6_HCW_S01_28Jan20220.csv' 
bpod_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D4_D5_D6_HCW1/DI1_D6_HCW_S01_28Jan20220.csv' 
df_nph = pd.read_csv(nph_path)
df_bpod = pd.read_csv(bpod_path) 
regions = 'Region5G' #SPECIAL CASE CUTTING 950: 
...
#D1 HCW2
HCW='HCW2'
eid = 'a438298c-5484-47aa-815c-01c92960fe85' 
nph_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW2/PhotometryData_D1_HCW_S02_03Nov2021.csv' 
bpod_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW2/DI1_HCW_S02_03Nov2021.csv'
df_nph = pd.read_csv(nph_path)
df_bpod = pd.read_csv(bpod_path) 
regions = 'Region1G'
...
#D1 HCW3
HCW='HCW3'
eid = 'a08a36d0-ca61-4e8a-91d4-16547cc2f49e'
nph_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW3/PhotometryData_D1_HCW_S03_04Nov2021.csv' 
bpod_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW3/DI1_D1_HCW_S03_04Nov2021.csv'
df_nph = pd.read_csv(nph_path)
df_bpod = pd.read_csv(bpod_path) 
regions = 'Region1G'
...
#D2 
HCW='HCW2'
eid = '598da55c-3ac1-4014-a51b-298a680ed71e' 
nph_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW2/PhotometryData_D1_HCW_S02_03Nov2021.csv' 
bpod_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW2/DI1_HCW_S02_03Nov2021.csv'
df_nph = pd.read_csv(nph_path)
df_bpod = pd.read_csv(bpod_path) 
...
#D2 
HCW='HCW3'
eid = '20c26bfe-66d9-4ac7-8a27-c9982e61931e' 
nph_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW3/PhotometryData_D2_HCW_S03_04Nov2021.csv'
bpod_path = '/mnt/h0/kb/data/HCW_KB_Nov2024/Sessions/D1_D2_D3_HCW3/DI1_D2_HCW_S03_04Nov2021.csv' 
df_nph = pd.read_csv(nph_path)
df_bpod = pd.read_csv(bpod_path) 
regions = 'Region1G'
...
#%%
""" Special situation for HCW3 M1 """ 
""" signal jumps so I cut the session into 3 parts """
# df_nph1 = df_nph[0:30365] 
# df_nph2 = df_nph[30450:43000]
# df_nph3 = df_nph[46550:54100]

""" Special situation for HCW1 D6 Region5G """ 
""" signal jumps a lot in the beginning """
# df_nph1 = df_nph[950:] 
# df_nph=df_nph1.reset_index(drop=True)
...
""" SAVE THE DATA """
df_nph.to_parquet(f'/mnt/h0/kb/data/HCW_KB_Nov2024/{mouse}_{date}_{regions}_{eid}.pqt')
df_trials.to_parquet(f'/mnt/h0/kb/data/HCW_KB_Nov2024/behav_{mouse}_{date}_{regions}_{eid}.pqt') 

# df_nph = pd.read_parquet(f'/mnt/h0/kb/data/HCW_KB_Nov2024/{mouse}_{date}_{regions}_{eid}.pqt') 
# df_trials = pd.read_parquet(f'/mnt/h0/kb/data/HCW_KB_Nov2024/behav_{mouse}_{date}_{regions}_{eid}.pqt')
...
# #%% 
# """ TO STOP THE EVENTS LIST IF THEY ARE NOT PART OF THE CROPPED PHOTOMETRY DATA """
# # Find the first index where repetition starts
# stop_idx = np.where(np.diff(idx_event) == 0)[0][0] + 1

# # Slice the array up to the first repeating value
# unique_idx_event = idx_event[:stop_idx-1]

# print(unique_idx_event)

# idx_event = unique_idx_event 

PERIEVENT_WINDOW = [-1,2] #never to be changed!!! "constant" 
SAMPLING_RATE = int(round(1 / np.mean(np.diff(array_timestamps_bpod)), 0)) 
...
n_trials = df_nph.trial_number[len(df_nph.trial_number)-1]
...
event_stimOnT = np.array(df_trials[EVENT][0:n_trials]) #pick the feedback timestamps 
...
psth_good = df_nph.calcium_mad.values[psth_idx] 

psth_good_avg = psth_good.mean(axis=1)
plt.plot(psth_good_avg, c='red') 
plt.axvline(x=(SAMPLING_RATE * abs(PERIEVENT_WINDOW[0])), c='gray', linestyle='dashed')
...
# Step 1: Compute mean calcium signal per trial
mean_calcium_per_trial = (
    df_nph.groupby("trial_number")["calcium_jove2019"]
    .mean()
    .reset_index(name="mean_calcium_jove2019")
)

# Step 2: Merge with df_trials to align trial information
df_correlation = df_trials.merge(
    mean_calcium_per_trial, left_index=True, right_on="trial_number"
)

# Step 3: Compute correlation
stim_on_screen = df_correlation["stimOnScreenT"]  # Stimulus on-screen times
mean_calcium = df_correlation["mean_calcium_jove2019"]  # Mean calcium signal
correlation = stim_on_screen.corr(mean_calcium)
print(f"Correlation between stimOnScreenT and mean calcium signal: {correlation:.3f}")

# Step 4: Plotting
plt.figure(figsize=(10, 8))

# Normalize trial numbers for colormap
trial_numbers = df_correlation["trial_number"]
norm = plt.Normalize(trial_numbers.min(), trial_numbers.max())  # Normalize trial numbers
colors = cm.viridis(norm(trial_numbers))  # Map trial numbers to viridis colors
...
cbar.set_label("Trial Number", rotation=270, labelpad=15)

# Add labels and titles
plt.xlabel("Stimulus On-Screen Time (s)")
plt.ylabel("Mean Calcium Signal (jove2019)")
plt.suptitle(f"{mouse} {date} {regions} {eid}")
plt.title("Correlation between Stimulus On-Screen Time and Calcium Signal")
plt.legend() 
plt.show()


#%% 
# Compute mean and error (e.g., standard error of the mean)
psth_good_mean = psth_good.mean(axis=1)
psth_good_sem = psth_good.std(axis=1) / np.sqrt(psth_good.shape[1])  # Standard error

# Time points for the x-axis
time_points = np.linspace(PERIEVENT_WINDOW[0], PERIEVENT_WINDOW[1], len(psth_good_mean))

# Compute y-axis limits (5th and 95th percentile)
y_min, y_max = np.percentile(psth_good, [5, 95])

# Plot mean PSTH
plt.figure(figsize=(8, 8))
plt.plot(time_points, psth_good_mean, color='red', label='Mean PSTH')
plt.plot(time_points, psth_good, linewidth=0.5, alpha=0.1, c='gray')

# Add shadowed area for error
plt.fill_between(
    time_points,
    psth_good_mean - psth_good_sem,  # Lower bound of error
    psth_good_mean + psth_good_sem,  # Upper bound of error
    color='red',
    alpha=0.3,
    label='SEM'
)

# Add vertical line for zero point
plt.axvline(x=0, color='gray', linestyle='dashed', label='Event Time')
...
plt.title(f"PSTH with Error {mouse} {date} {regions} {eid}")
plt.legend()
plt.show() 
...
event_list = ["stimOnTrigger_times", "feedback_times"]
PERIEVENT_WINDOW = [-1, 2]  # Constant perievent window
array_timestamps_bpod = np.array(df_nph.times)  # NPH timestamps in bpod clock
SAMPLING_RATE = int(round(1 / np.mean(np.diff(array_timestamps_bpod)), 0))

# Prepare the figure: 2 rows, 3 columns
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
axes = axes.flatten()  # Flatten to index easily

for i, event in enumerate(event_list):
    # Process each event
    event_test = np.array(df_trials[event])  # Event timestamps
    idx_event = np.searchsorted(array_timestamps_bpod, event_test)

    sample_window = np.arange(PERIEVENT_WINDOW[0] * SAMPLING_RATE, PERIEVENT_WINDOW[1] * SAMPLING_RATE + 1)
    n_trials = df_nph.trial_number.iloc[-1]
    psth_idx = np.tile(sample_window[:, np.newaxis], (1, n_trials))
    event_timestamps = df_trials[event][:n_trials]
    stimOnT_idx = np.searchsorted(array_timestamps_bpod, event_timestamps)
    psth_idx += stimOnT_idx

    psth_good = df_nph.calcium_mad.values[psth_idx]
    psth_good_mean = psth_good.mean(axis=1)
    psth_good_sem = psth_good.std(axis=1) / np.sqrt(psth_good.shape[1])
    time_points = np.linspace(PERIEVENT_WINDOW[0], PERIEVENT_WINDOW[1], len(psth_good_mean))

    # --- PSTH Plot ---
    ax_psth = axes[i * 3]  # Place PSTH in the leftmost columns
    ax_psth.plot(time_points, psth_good_mean, color='red', label='Mean PSTH') #9d53ff
    ax_psth.plot(time_points, psth_good, linewidth=0.5, alpha=0.1, color='gray')
    ax_psth.fill_between(
        time_points,
        psth_good_mean - psth_good_sem,
        psth_good_mean + psth_good_sem,
        color='red', #9d53ff
        alpha=0.3,
        label='SEM'
    )
    ax_psth.axvline(x=0, color='gray', linestyle='dashed', label='Event Time')
    ax_psth.set_ylim(np.percentile(psth_good, [5, 95]))
    ax_psth.set_title(f"{event} - PSTH")
    ax_psth.set_xlabel("Time (s)")
    ax_psth.set_ylabel("Signal (a.u.)")
    ax_psth.legend()

# --- Scatterplots ---
# Scatter 1: stimOnScreenT vs mean_calcium_jove2019
mean_calcium_per_trial = (
    df_nph.groupby("trial_number")["calcium_jove2019"]
    .mean()
    .reset_index(name="mean_calcium_jove2019")
...
stim_on_screen = df_correlation["stimOnScreenT"]
mean_calcium = df_correlation["mean_calcium_jove2019"]
correlation_stim = stim_on_screen.corr(mean_calcium)

trial_numbers = df_correlation["trial_number"]
colors = cm.viridis(plt.Normalize(trial_numbers.min(), trial_numbers.max())(trial_numbers))

ax_scatter1 = axes[1]  # Top-middle subplot
scatter1 = ax_scatter1.scatter(
    ...
    cmap="viridis",
    alpha=0.7,
    label=f'r={correlation_stim:.3f}'
)
cbar1 = fig.colorbar(scatter1, ax=ax_scatter1)
cbar1.set_label("Trial Number", rotation=270, labelpad=15)
ax_scatter1.set_title("stimOnScreenT - Scatter Plot")
ax_scatter1.set_xlabel("Stimulus On-Screen Time (s)")
ax_scatter1.set_ylabel("Mean Calcium Signal (jove2019)")
ax_scatter1.legend()

# Scatter 2: allSContrasts vs mean_calcium_jove2019
all_contrasts = df_correlation["allSContrasts"]
correlation_contrasts = all_contrasts.corr(mean_calcium)

ax_scatter2 = axes[2]  # Top-right subplot
scatter2 = ax_scatter2.scatter(
    ...
    cmap="viridis",
    alpha=0.7,
    label=f'r={correlation_contrasts:.3f}'
)
cbar2 = fig.colorbar(scatter2, ax=ax_scatter2)
cbar2.set_label("Trial Number", rotation=270, labelpad=15)
ax_scatter2.set_title("allSContrasts - Scatter Plot")
ax_scatter2.set_xlabel("All Contrasts")
ax_scatter2.set_ylabel("Mean Calcium Signal (jove2019)")
ax_scatter2.legend()

# --- Correlation Matrix ---
# Extract relevant behavior variables and photometry signal for correlation
behavior_vars = [
    'rewardVolume', 'stimOff_times', 'allContrasts', 'stimOn_times',
    'stimOnScreenT', 'feedbackType', 'goCue_times'
]
data_for_corr = df_trials[behavior_vars].copy()
data_for_corr["calcium_jove2019"] = mean_calcium

# Compute correlation matrix
corr_matrix = data_for_corr.corr()

# Plot heatmap
ax_corr = axes[4]  # Bottom-middle subplot
sns.heatmap(corr_matrix, annot=True, cmap="coolwarm", ax=ax_corr, fmt=".2f")
ax_corr.set_title("Correlation Matrix (Behavior vs. Calcium)")

# --- New Plot: PSTH Split by `allSContrasts` ---
event = "feedback_times"  # Use feedback_times
contrasts = df_trials["allSContrasts"].unique()  # Get unique contrast values
contrasts = np.sort(contrasts)  # Ensure consistent order
colors = cm.viridis(np.linspace(0, 1, len(contrasts)))  # Assign a color for each contrast
colors2 = cm.PiYG(np.linspace(0, 1, len(contrasts)))  # Assign a color for each contrast

ax_split = axes[5]  # Bottom-right subplot
for contrast, color in zip(contrasts, colors2):
    # Filter trials by contrast
    contrast_trials = df_trials[df_trials["allSContrasts"] == contrast]
    event_timestamps = contrast_trials[event]
    n_contrast_trials = len(event_timestamps)

    # Calculate PSTH for each contrast
    psth_idx = np.tile(sample_window[:, np.newaxis], (1, n_contrast_trials))
    stimOnT_idx = np.searchsorted(array_timestamps_bpod, event_timestamps)
    psth_idx += stimOnT_idx
...
ax_split.axvline(x=0, color='gray', linestyle='dashed', label='Event Time')
ax_split.set_title(f"PSTH Split by allSContrasts\n({event})")
ax_split.set_xlabel("Time (s)")
ax_split.set_ylabel("Signal (a.u.)")
ax_split.legend(title="Contrast", fontsize=8)

# Adjust layout and add shared title
fig.suptitle(f"{HCW} {mouse} {date} {regions} {eid} - Event-Based Analysis", fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.96])  # Leave space for the suptitle
plt.savefig(f'/mnt/h0/kb/data/HCW_KB_Nov2024/FigHCW2_{EVENT}_{HCW}_{mouse}_{date}_{regions}_{eid}.png')
plt.show()
...
for name in video_column_names: 
    ...
    plt.xlim(-5, 81)
    plt.title(f'Calcium and {name} over Time')
    plt.show()
