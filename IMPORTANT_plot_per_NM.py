...
for i in range(len(df_goodsessions)): 
    try: 
        ...
        dataset_task_settings = one.load_dataset(eid, '_iblrig_taskSettings.raw.json')  
        values = dataset_task_settings.get('BLOCK_INIT_5050', 'Key not found') 

        if values == True: #filter to have only BCW sessions 
        ...
        print("ERROR: ", i)
        EXCLUDES.append(i) #run 20240113 len(EXCLUDES) = 42
#%%
    ##################################################################################################
...
# Assuming df_trials_combined is already loaded

# Grouping by probL and calculating % correct
grouped = df_trials_combined.groupby('probL')['feedbackType'].apply(lambda x: (x == 1).mean() * 100).reset_index()
grouped.columns = ['probL', 'percent_correct']

# Plotting the results
plt.figure(figsize=(5, 5))
plt.bar(grouped['probL'], grouped['percent_correct'], color=['blue', 'orange', 'green'], alpha=0.5)
plt.title('Percentage of Correct Answers by probL')
plt.xlabel('probL')
plt.ylabel('Percentage of Correct Answers')
plt.xticks(grouped['probL'])
plt.ylim(0, 100)
plt.show()

# Extracting correct rates for each group
group_data = [df_trials_combined.loc[df_trials_combined['probL'] == prob, 'feedbackType'] == 1 for prob in grouped['probL']]

# Performing statistical test (Kruskal-Wallis)
...
posthoc = posthoc_dunn(
    [df_trials_combined.loc[df_trials_combined['probL'] == prob, 'feedbackType'] == 1 for prob in grouped['probL']],
    p_adjust='bonferroni'
)
...
# Create a heatmap for Dunn's test results
plt.figure(figsize=(8, 6))
sns.heatmap(posthoc, annot=True, fmt=".3f", cmap="coolwarm", xticklabels=grouped['probL'], yticklabels=grouped['probL'])
...
#%%
""" same as prev but for stitmOn, added 10092024 """
""" 1.2.1 stimOn correct inc 4NMs """ 
# Create the figure and gridspec
fig = plt.figure(figsize=(12, 12))
gs = fig.add_gridspec(2, 2, height_ratios=[3, 3])
...
# Plot for DA
ax1 = fig.add_subplot(gs[0, 0])
ax1.plot(psth_correct_avg_DA_s, color='#2f9c95', linewidth=3, label='Correct')
ax1.fill_between(range(len(psth_correct_avg_DA_s)), psth_correct_avg_DA_s - sem_correct_DA_s, psth_correct_avg_DA_s + sem_correct_DA_s, color='#2f9c95', alpha=0.15)
ax1.plot(psth_incorrect_avg_DA_s, color='#d62828', linewidth=3, label='Incorrect')
ax1.fill_between(range(len(psth_incorrect_avg_DA_s)), psth_incorrect_avg_DA_s - sem_incorrect_DA_s, psth_incorrect_avg_DA_s + sem_incorrect_DA_s, color='#d62828', alpha=0.15)
ax1.axvline(x=30, color="black", alpha=0.9, linewidth=3, linestyle="dashed")
ax1.set_ylabel('Average Value')
ax1.set_xlabel('Time (s)')
ax1.set_title('DA')
...
# Plot for 5HT
ax2 = fig.add_subplot(gs[0, 1], sharex=ax1, sharey=ax1)
ax2.plot(psth_correct_avg_5HT_s, color='#2f9c95', linewidth=3, label='Correct')
ax2.fill_between(range(len(psth_correct_avg_5HT_s)), psth_correct_avg_5HT_s - sem_correct_5HT_s, psth_correct_avg_5HT_s + sem_correct_5HT_s, color='#2f9c95', alpha=0.15)
ax2.plot(psth_incorrect_avg_5HT_s, color='#d62828', linewidth=3, label='Incorrect')
ax2.fill_between(range(len(psth_incorrect_avg_5HT_s)), psth_incorrect_avg_5HT_s - sem_incorrect_5HT_s, psth_incorrect_avg_5HT_s + sem_incorrect_5HT_s, color='#d62828', alpha=0.15)
ax2.axvline(x=30, color="black", alpha=0.9, linewidth=3, linestyle="dashed")
ax2.set_ylabel('Average Value')
ax2.set_xlabel('Time (s)')
ax2.set_title('5HT')
...
# Plot for NE
ax3 = fig.add_subplot(gs[1, 0], sharex=ax1, sharey=ax1)
ax3.plot(psth_correct_avg_NE_s, color='#2f9c95', linewidth=3, label='Correct')
ax3.fill_between(range(len(psth_correct_avg_NE_s)), psth_correct_avg_NE_s - sem_correct_NE_s, psth_correct_avg_NE_s + sem_correct_NE_s, color='#2f9c95', alpha=0.15)
ax3.plot(psth_incorrect_avg_NE_s, color='#d62828', linewidth=3, label='Incorrect')
ax3.fill_between(range(len(psth_incorrect_avg_NE_s)), psth_incorrect_avg_NE_s - sem_incorrect_NE_s, psth_incorrect_avg_NE_s + sem_incorrect_NE_s, color='#d62828', alpha=0.15)
ax3.axvline(x=30, color="black", alpha=0.9, linewidth=3, linestyle="dashed")
ax3.set_ylabel('Average Value')
ax3.set_xlabel('Time (s)')
ax3.set_title('NE')
...
# Plot for ACh
ax4 = fig.add_subplot(gs[1, 1], sharex=ax1, sharey=ax1)
ax4.plot(psth_correct_avg_ACh_s, color='#2f9c95', linewidth=3, label='Correct')
ax4.fill_between(range(len(psth_correct_avg_ACh_s)), psth_correct_avg_ACh_s - sem_correct_ACh_s, psth_correct_avg_ACh_s + sem_correct_ACh_s, color='#2f9c95', alpha=0.15)
ax4.plot(psth_incorrect_avg_ACh_s, color='#d62828', linewidth=3, label='Incorrect')
ax4.fill_between(range(len(psth_incorrect_avg_ACh_s)), psth_incorrect_avg_ACh_s - sem_incorrect_ACh_s, psth_incorrect_avg_ACh_s + sem_incorrect_ACh_s, color='#d62828', alpha=0.15)
ax4.axvline(x=30, color="black", alpha=0.9, linewidth=3, linestyle="dashed")
ax4.set_ylabel('Average Value')
ax4.set_xlabel('Time (s)')
ax4.set_title('ACh')
...
fig.suptitle('Neuromodulator activity at Visual Stim Onset', y=1.02, fontsize=16)
plt.tight_layout()
plt.show()
##################################################################################################
#%% 
...
def plot_neuromodulator(ax, psth_combined, df_trials, title):
    # Filter to include only correct trials
    correct_trials_mask = df_trials.feedbackType == 1 #change here
    df_trials_correct = df_trials[correct_trials_mask]
